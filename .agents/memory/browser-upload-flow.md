...
Upload controls must be tested beyond the file picker UI: the selected file needs an onChange handler that moves it into the actual feature flow. When there is no upload backend, a validated local data URL plus a short-lived browser handoff can support private editing without pretending the file was uploaded.
